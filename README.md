...
- **엔진** — *Metal Flow* (기본): 자체 GPU 파이프라인(피라미드 옵티컬 플로우 + 풀해상도 워프), 배율 자유, 원본 선명도 유지. *Apple FI*: 애플 ANE 모델, 720p·2× 고정; 디스플레이를 fps × 2로 설정(60→120, 24→144).
...
- `Sources/Interpolation` — 엔진(`MetalFlowEngine`, `AppleFIEngine`, `PairEngine` 프로토콜) + `InterpBench`(결정적 PSNR/타이밍 회귀 도구)
